    ...
    def test_decodes_base64_content(self, monkeypatch):
        ...
        respx.get("https://api.github.com/repos/o/r/contents/README.md").mock(
            return_value=httpx.Response(
                200, json={"name": "README.md", "content": encoded, "type": "file"}
            )
        )
...
class TestCreateBranch:
    ...
    def test_posts_ref(self, monkeypatch):
        ...
        route = respx.post("https://api.github.com/repos/o/r/git/refs").mock(
            return_value=httpx.Response(
                201, json={"ref": "refs/heads/feat/x", "object": {"sha": "abc"}}
            )
        )
...
class TestDeleteFile:
    @respx.mock
    def test_calls_delete_with_payload(self, monkeypatch):
        import json as _json
        ...
        importlib.reload(m)
        route = respx.delete("https://api.github.com/repos/o/r/contents/old.py").mock(
            return_value=httpx.Response(200, json={"commit": {"sha": "xyz"}})
        )
        result = m.github_delete_file("o", "r", "old.py", "remove old.py", "deadbeef", "main")
        body = _json.loads(route.calls[0].request.content)
        assert body["message"] == "remove old.py"
        assert body["sha"] == "deadbeef"
        assert body["branch"] == "main"
...
class TestCloseIssue:
    ...
    def test_adds_comment_before_close(self, monkeypatch):
        ...
        comment_route = respx.post("https://api.github.com/repos/o/r/issues/3/comments").mock(
            return_value=httpx.Response(201, json={"id": 1, "body": "Done!"})
        )
        respx.patch("https://api.github.com/repos/o/r/issues/3").mock(
            return_value=httpx.Response(200, json={"number": 3, "state": "closed"})
        )
        result = m.github_close_issue("o", "r", 3, comment="Done!")
